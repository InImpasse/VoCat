...
白名单弱化为只匹配 VID/PID。每个登记的宿主槽位还会固定映射到对应的来宾 USB
端口，因此重插顺序不会再交换来宾设备 ID。当前 qemu-xhci 配置最多支持 14 台
已登记模块。没有 USB 序列号时，只能识别物理端口，不能识别被移动到其他端口的
模块本体；需要人类可读标识时使用现有的自定义设备名称。
...
已登记设备后续会自动热插拔。已有安装升级到固定来宾 USB 槽位时，`--apply` 会
保留当前附件；随后需要一次性拔出全部已登记模块，再按任意顺序插回，使旧的自动
分配地址先全部释放。开始调制解调器测试前，必须对每台设备分别验证
