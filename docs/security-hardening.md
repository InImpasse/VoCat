...
`.gitleaks.toml` extends the upstream Gitleaks rules. Its only project allowlist requires the generic-key rule, the exact Tailscale public APT signing fingerprint assignment, and its deployment-script path to match together. Do not broaden it to a whole path or a token-shaped prefix.
...
Written permission covering the intended use, region, and duration was confirmed on 2026-08-24. The permission document and its sensitive terms remain outside the repository.
